...
# It should be sourced by other build scripts to perform common setup tasks.
...
# (Your env debugging command remains the same)
env | grep -E '^(WEB_URL|...|firebase_config)' | sort # Truncated for brevity

# 🔧 Sanitizing variables
clean_known_vars() {
  echo "⚠️ Cleaning variables known to contain invisible Unicode..."
...
#
## This script is not meant to be executed directly.
## It should be sourced by other build scripts.
#
#set -euo pipefail
#
#echo "🧹 Running flutter clean..."
...
#
## 🔧 Sanitizing only critical known variables
#clean_known_vars() {
...
#}
#
## Export the function and its result so it's available in the calling script
#export DART_DEFINES
